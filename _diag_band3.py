...
async def main():
    ...
    async with websockets.connect(t["webSocketDebuggerUrl"], max_size=90*1024*1024) as ws:
        ...
        js = """(() => { const b=document.querySelector('.brand-band'); const r=b.getBoundingClientRect();
              return JSON.stringify({x:0, y: Math.round(r.top+scrollY-120), w:1440, h: Math.round(r.height+240)}); })()"""
        ...
        open(r"C:/Users/Orange/workspace/curve-site/brand_check/diag_band3.png","wb").write(base64.b64decode(shot["data"]))
        print("OK diag_band3.png")
asyncio.run(main())
